...
static struct {
    ...
    jfieldID ad_id_field, ad_hamming_field, ad_decision_margin_field, ad_c_field, ad_p_field;
} state;

JNIEXPORT void JNICALL Java_com_protondx_dragonfly_gmm_ApriltagNative_native_1init
    (JNIEnv *env, jclass cls)
{
    ...
    jclass ad_cls = (*env)->FindClass(env, "com/protondx/dragonfly/gmm/ApriltagDetection");
    if (!ad_cls) {
        __android_log_write(ANDROID_LOG_ERROR, "apriltag_jni",
    ...
    state.ad_decision_margin_field = (*env)->GetFieldID(env, ad_cls, "decision_margin", "I");
 ...
 */
JNIEXPORT void JNICALL Java_com_protondx_dragonfly_gmm_ApriltagNative_yuv_1to_1rgb
 ...
 */
JNIEXPORT void JNICALL Java_com_protondx_dragonfly_gmm_ApriltagNative_apriltag_1init
    ...
    // Initialize state
    const char *tfname = "tag16h5";//(*env)->GetStringUTFChars(env, _tfname, NULL);
 ...
 */
JNIEXPORT jobject JNICALL Java_com_protondx_dragonfly_gmm_ApriltagNative_apriltag_1detect_1yuv
        (JNIEnv *env, jclass cls, jbyteArray _buf, jint width, jint height) {
    // If not initialized, init with default settings
    if (!state.td) {
        state.tf = tag16h5_create();//tag36h11_create();
        state.td = apriltag_detector_create();
    ...
    for (int i = 0; i < zarray_size(detections); i += 1) {
        ...
        (*env)->SetIntField(env, ad, state.ad_decision_margin_field, det->decision_margin);
